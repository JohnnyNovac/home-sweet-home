...
с именем из Яндекса). Имя берётся из Яндекса только при первом появлении лампы; если потом переименовать её вручную,
синхронизация это имя больше не перезапишет.
...
Если светом управляли в обход системы (например, голосом, пока в комнате никого не было), presence-service заметит это
сам: при первом присутствии после паузы дольше 90 секунд он сверяет своё представление о лампе с реальным состоянием в
Яндексе и дальше действует по сверенному. Порог задаётся настройкой `app.lamp.lamp-state-sync-gap`.
