...
**Планируется:** порог покрытия в CI через `jacocoTestCoverageVerification` (`violationRules`), чтобы сборка падала
при просадке покрытия. Порог задавать по ветвям; DTO, config, `*Application` и недостижимые ветви (например,
синтетический `default` в `switch` по enum) исключать из проверки.